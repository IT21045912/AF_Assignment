...
const useStyles = makeStyles((theme) => ({
  ...
  function ShowCardVeiew() {
    ...
    useEffect(() => {
      axios.get('http://localhost:1337/api/harvest-controller/')
      .then((res) => {
            setHarvest(res.data.Harvests);
            console.log(res.data.Harvests)
      })
        .catch((err) => {
          alert(err);
        }).then((d) => {
    ...
    return (
      <>
        {/* <Container style={{ backgroundColor: 'white', width: '90%', marginTop: '20px', padding: '20px', borderRadius: '15px' }}> */}
